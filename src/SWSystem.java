...
public class SWSystem {
    ...
    public void printReport() {
        ...
        for (QualityDimension d : dimensions) {
            System.out.println("\n--- " + d.getName() + " [" + d.getIsoCode() + "] (Weight: " + d.getWeight() + ") ---");

            for (Criterion c : d.getCriteria()) {
                System.out.println(c.getName() + ": " + c.getMeasuredValue() + " " + c.getUnit() +
                        " -> Score: " + c.calculateScore() + " (" +
                        (c.getDirection().equals("higher") ? "Higher is better" : "Lower is better") + ")");
            }

            System.out.println(">> Dimension Score: " + d.calculateDimensionScore() + "/5 [" + d.getLabel() + "]");
        }

        double overall = calculateOverallScore();
        String overallLabel = overall >= 4.5 ? "Excellent Quality" :
                overall >= 3.5 ? "Good Quality" :
                        overall >= 2.5 ? "Needs Improvement" : "Poor Quality";
        ...
        System.out.println("Level: " + weakest.getLabel());
